    ...
    walk: function walk(callback) {
      self.visitFeatures(features, function() {
        ...
        supportCodeLibrary.instantiateNewWorld(function(world) {
          ...
          var event   = AstTreeWalker.Event(AstTreeWalker.AFTERALL_EVENT_NAME, payload);
          var hookedUpAfterAll = supportCodeLibrary.afterAllHooksFunction(world);
          self.broadcastEventAroundUserFunction(
            event,
            hookedUpAfterAll,
            function() { callback(featuresResult); }
          );
        });
      });
    },
...
AstTreeWalker.AFTERALL_EVENT_NAME                 = 'AfterAll';
