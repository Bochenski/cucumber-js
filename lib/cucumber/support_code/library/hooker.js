var Hooker = function() {
  ...
  var afterAllHooks = Cucumber.Type.Collection();
  ...
  var self = {
    ...
    addAfterAllHookCode: function addAfterAllHookCode(code, options) {
      var afterAllHook = Cucumber.SupportCode.Hook(code, options);
      afterAllHooks.unshift(afterAllHook);
    },
    ...
    triggerAfterAllHooks: function triggerAfterAllHooks(world, callback) {
      afterAllHooks.forEach(function(afterAllHook, callback) {
        afterAllHook.invoke(world, callback);
      }, callback);
    }
